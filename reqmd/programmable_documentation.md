...
다음 문서로 전파할 때는 이 `changed_content`를 target 문서의 `incoming_change`로 넣고, `document`, `source_doc`, `direction`, `reason/trace`를 붙여 Queue item을 만듭니다.

따라서 값의 흐름은 다음과 같습니다.

```text
incoming_change -> UpdateContent(document, incoming_change, direction) -> changed_content
source changed_content + target document + direction + source_doc + reason/trace -> Queue item
...
Queue item은 처리할 문서, 그 문서에 전달할 `incoming_change`, 변경 방향, 추적 정보를 담는 작업 단위입니다.
...
- `reason/trace`: 이 item이 생성된 이유와 추적 근거입니다.
