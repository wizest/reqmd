...
- RequirementSection의 섹션 **제목**은 반드시 식별자로 시작해야 합니다.
- GeneralSection은 식별자로 시작하지 않으며 `@.md`, `=.md` 색인의 대상이 아닙니다.
...
- `#fragment`: 색인 파일 안의 특정 섹션 위치를 지정합니다. 요구사항 본문에서는 생략할 수 있으나, `@.md` 색인 파일 안에서는 반드시 작성합니다. 색인 본문 목록의 fragment는 대상 색인 섹션 heading의 표시 텍스트를 반영한 VS Code 호환 anchor 문자열이어야 합니다.
...
식별자 색인 섹션 제목은 원래 요구사항 위치를 가리키는 링크로 작성합니다. 제목 링크는 **원문 요구사항의 단일 섹션**으로만 이동해야 합니다.
...
GeneralSection
  "<General Markdown section that is not indexed and does not start with an identifier>"
```
...
`@.md`의 각 섹션은 하나의 `RequirementSection` 식별자를 기준으로 작성합니다. `GeneralSection`은 포함하지 않습니다:
...
`=.md`의 각 섹션은 하나의 `RequirementSection` 도움자를 기준으로 작성합니다. `GeneralSection`은 포함하지 않습니다:
