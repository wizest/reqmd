...
- 같은 수준의 문서 관계가 새로 생기거나 변경되면, 해당 요구사항 간 lateral 관계를 ReqMd 식별자 색인 `@.md` 규칙에 따라 반영합니다.
...
- 현재 요구사항이 상위 요구사항과 새로 연결되거나 연결이 바뀌면, upstream 관계를 ReqMd 식별자 색인 `@.md` 규칙에 따라 반영합니다.
...
- 현재 요구사항에서 파생되는 하위 요구사항이나 산출물 관계가 바뀌면, downstream 관계를 ReqMd 식별자 색인 `@.md` 규칙에 따라 반영합니다.
...
색인 갱신은 ReqMd의 식별자 색인 `@.md` 규칙을 따릅니다.
즉, 요구사항 ID별 index section과 source section fragment, index section 간 링크 형식을 ReqMd `@.md` 규칙에 맞게 유지해야 합니다.

에이전트는 전파 대상 문서를 수정한 뒤 다음 사항을 확인합니다.

- lateral 전파로 같은 수준의 요구사항 또는 문서 섹션이 연결되었는지 확인하고, 필요한 색인 링크를 `@.md` 규칙에 맞게 갱신합니다.
- upstream 전파로 현재 요구사항이 더 상위 요구사항, 정책, 아키텍처 결정과 연결되었는지 확인하고, 상위 방향 추적성을 `@.md` 규칙에 맞게 갱신합니다.
- downstream 전파로 현재 요구사항에서 파생된 상세 설계, 구현, 테스트, 운영 요구사항이 바뀌었는지 확인하고, 하위 방향 추적성을 `@.md` 규칙에 맞게 갱신합니다.
...
문서 갱신으로 요구사항 간 관계가 바뀌면, 에이전트는 ReqMd 식별자 색인 `@.md` 규칙에 따라 색인을 함께 갱신하여 propagation 결과가 추적성 구조에 남도록 합니다.
...
8. 전파 과정에서 lateral, upstream, downstream 요구사항 관계가 새로 식별되거나 변경되면 ReqMd 식별자 색인 `@.md` 규칙에 따라 색인을 갱신합니다.
...
ProDoc skill은 사용자가 ProDoc 문서를 작성, 수정, 검증, 전파하거나 ReqMd 식별자 색인을 함께 갱신하도록 요청할 때 사용합니다.
...
- 전파 과정에서 확인된 `lateral`, `upstream`, `downstream` 관계를 ReqMd 식별자 색인에 반영하는 요청
...
- 전파 과정에서 변경된 `lateral`, `upstream`, `downstream` 관계가 ReqMd 식별자 색인 `@.md` 규칙에 따라 갱신되어 있다.
