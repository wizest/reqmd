...
Run `python .codex\skills\reqmd\scripts\validate_reqmd.py <root>` when possible. Use this checklist for manual review or when script output needs interpretation.

Treat validation output as the primary repair guide. Run `python .codex\skills\reqmd\scripts\reqmd_fix.py <root>` before hand-editing indexes. If validation still reports `REPAIRABLE`, run the named script once. Treat `REVIEW` as semantic or unresolved traceability work.
