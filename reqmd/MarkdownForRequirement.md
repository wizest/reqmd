...
- `@.md`, `=.md` 색인 파일 안의 식별자/도움자 링크는 반드시 `@#fragment`, `=#fragment`, `path/@#fragment`, `path/=#fragment`처럼 섹션 fragment를 포함합니다. fragment는 생략하지 않으며, VS Code가 이해하는 Markdown heading anchor 형식으로 작성합니다.
  - fragment는 대상 heading의 표시 텍스트만 반영합니다. 링크 텍스트 `[SW_BLC_CONTROL](@)`나 `[brake_lamp_request](=)`는 fragment 계산에 포함하지 않습니다.
  - 예: `## [SW_BLC_CONTROL](@) 브레이크 램프 제어 소프트웨어` -> `SwReq.md#sw_blc_control-브레이크-램프-제어-소프트웨어`
  - 예: `## [brake_lamp_request](=) 브레이크 램프 요청` -> `SwDesign.md#brakelampreq-브레이크-램프-요청`
...
- `#fragment`: 색인 파일 안의 특정 섹션 위치를 지정합니다. 요구사항 본문에서는 생략할 수 있으나, `@.md` 색인 파일 안에서는 반드시 작성합니다. fragment는 대상 섹션의 heading 전체를 반영한 VS Code 호환 anchor 문자열이어야 합니다. 링크 텍스트가 아니라 heading의 표시 텍스트를 기준으로 anchor를 작성합니다.
...
색인 파일 안에서는 `[identifier](@)` 또는 `[identifier](pathto/@)`처럼 파일만 가리키는 링크를 사용하지 않습니다. 같은 색인 파일의 다른 섹션은 `[identifier](@#fragment)`로, 다른 경로의 색인 섹션은 `[identifier](pathto/@#fragment)`로 작성합니다. fragment는 대상 heading 전체를 반영한 VS Code 호환 anchor 문자열이어야 하며, 예를 들어 `SwReq.md#sw_blc_control-브레이크-램프-제어-소프트웨어`처럼 작성합니다.
...
색인 파일 안에서는 `[helper](=)` 또는 `[helper](path/=)`처럼 파일만 가리키는 링크를 사용하지 않습니다. 같은 색인 파일의 다른 섹션은 `[helper](=#fragment)`로, 다른 경로의 색인 섹션은 `[helper](path/=#fragment)`로 작성합니다. fragment는 대상 heading 전체를 반영한 VS Code 호환 anchor 문자열이어야 하며, 예를 들어 `SwDesign.md#brakelampreq-브레이크-램프-요청`처럼 작성합니다. 구현 도움자도 같은 규칙을 따르며, 실제 구현 이름과 heading anchor를 함께 보존합니다.
...
`속성자`는 사용자 변수를 명시하기 위해 Markdown 코드블록을 사용하여 YAML 형식으로 정의됩니다. 이 변수들은 해당 요구사항의 식별자 내에서 유효합니다. 속성자는 요구사항 본문에만 작성합니다. `@.md`, `=.md` 색인 파일에는 YAML 속성자를 작성하지 않습니다.
...
한 요구사항 본문 안에서 속성자가 여러 번 나오고 같은 키가 반복되면 다음 규칙을 적용합니다:
...
`식별자 색인` 은 Markdown 파일로 작성되며, 각 식별자에 대한 관계(relationship) 정보를 표현합니다. 식별자 색인은 요구사항 간의 관계를 명확히 하기 위해 사용됩니다. 속성의 원천 정보는 요구사항 본문에만 두고, 식별자 색인에는 YAML 속성자를 작성하지 않습니다.

식별자 색인은 다음과 같은 형식으로 작성됩니다:

````markdown
## [식별자](SwReq.md#sw_blc_control-브레이크-램프-제어-소프트웨어)

- [관계있는 식별자1](@#관계있는_식별자1)
- [관계있는 식별자2](pathto/@#관계있는_식별자2)
````

식별자 색인 섹션 제목은 원래 요구사항 위치를 가리키는 링크로 작성합니다. 목록에는 해당 원래 위치 링크를 중복해서 쓰지 않고, 현재 섹션의 식별자와 관계가 있는 항목만 적습니다. 색인 파일 안에서 다른 색인 엔트리를 가리키는 링크는 반드시 fragment를 포함합니다. 관계의 종류는 링크 대상의 경로, 문서 위치, 주변 설명, 프로젝트 맥락으로 해석합니다. 색인 파일은 기본적으로 관계 타입을 강제하지 않습니다.

예:

```markdown
## [SW_BRAKE_LAMP_REQUEST](SwReq.md#sw_blc_control-브레이크-램프-제어-소프트웨어)

- [SYS_BRAKE_LAMP_CONTROL](../sys/@#sys_blc_control-브레이크-램프-제어-시스템)
- [SWQT_BLC_PEDAL_ON_001](../swqt/@#swqt_blc_pedal_on_001-브레이크-페달-눌림-테스트)
```
...
식별자 색인은 자동 생성하거나 갱신할 수 있는 파생 정보로 취급합니다. 따라서 사람이 직접 관리해야 하는 상태, 우선순위, 검증 방법 같은 속성은 색인이 아니라 원문 요구사항 섹션의 YAML에 작성합니다.


### 도움자 색인 (`=.md` 파일)

`도움자 색인` 은 Markdown 파일로 작성되며, 각 도움자에 대한 연결(mapping) 정보를 표현합니다. 도움자 색인은 도움자 간 연결을 명확히 하기 위해 사용됩니다. 도움자에 대한 속성이나 정의가 필요하면 원문 요구사항, 설계 문서, 테스트 문서 같은 본문에 작성하고, 도움자 색인에는 YAML 속성자를 작성하지 않습니다.

도움자 색인은 다음과 같은 형식으로 작성됩니다:


````markdown
## [도움자](SwDesign.md#brakelampreq-브레이크-램프-요청)

- [연결된_도움자1](=#연결된_도움자1)
- [연결된_도움자2](pathto/=#연결된_도움자2)
````

도움자 색인 섹션 제목은 원래 도움자 위치를 가리키는 링크로 작성합니다. 목록에는 해당 원래 위치 링크를 중복해서 쓰지 않고, 현재 섹션의 도움자와 연결된 항목만 적습니다. 색인 파일 안에서 다른 색인 엔트리를 가리키는 링크는 반드시 fragment를 포함합니다. 연결의 종류는 링크 대상의 경로, 문서 위치, 주변 설명, 프로젝트 맥락으로 해석합니다. 색인 파일은 기본적으로 연결 타입을 강제하지 않습니다.

예:

```markdown
## [brake_lamp_request](SwDesign.md#brakelampreq-브레이크-램프-요청)

- [brake_lamp_request_signal](../swdd/=#brake_lamp_request_signal-브레이크-램프-신호)
- [observed_brake_lamp_request](../swqt/=#observed_brake_lamp_request-관찰값)
```
...
색인 섹션에 포함된 링크는 모두 현재 섹션의 식별자 또는 도움자와 관계가 있는 항목으로 해석합니다. 색인 내부의 식별자/도움자 링크는 정의 섹션으로 바로 이동할 수 있도록 반드시 fragment를 포함합니다. 관계 종류는 기본 문법으로 강제하지 않으며, 경로와 주변 맥락으로 해석합니다.
...
- `@.md` 또는 `=.md`의 제목 링크가 실제 문서 위치와 heading anchor를 정확히 가리키는지 확인합니다.
...
Skill의 목적은 ReqMd 문법을 설명하는 것이 아니라, 에이전트가 요구사항 본문과 색인을 일관되게 읽고 수정하고 검증하도록 만드는 것입니다. 따라서 `SKILL.md`에는 긴 문법 설명보다 작업 절차, 판단 기준, 필요한 reference 선택 규칙을 우선 작성합니다.
...
- `scripts/update_index.py`: 본문에서 식별자와 도움자를 추출해 색인을 생성하거나 보강합니다.
...
3. 요구사항 본문을 하나의 검증 가능한 동작 또는 제약으로 작성합니다.
4. 필요한 도움자를 `snake_case` 또는 구현 도움자 형식으로 작성합니다.
5. YAML 속성자를 요구사항 본문에 작성합니다.
6. `@.md`에 새 식별자 섹션을 만들고 섹션 제목에 원래 위치 링크를 둡니다.
7. 상위 요구사항, 설계, 테스트와의 관계가 있으면 fragment 포함 링크로 추가합니다.
8. `=.md`에 새 도움자 섹션을 만들거나 기존 섹션을 보강합니다.

#### 기존 요구사항 수정

1. 수정 대상 식별자의 본문 섹션을 찾습니다.
2. 식별자 이름은 가능한 유지합니다.
3. 본문에서 추가되거나 제거된 도움자를 추출합니다.
...
- 요구사항 본문과 색인이 충돌하면 본문을 원천 정보로 보고 색인을 갱신 대상으로 봅니다.
- 속성자는 본문 YAML만 원천으로 인정하고, 색인에는 생성하지 않습니다.
...
- 요구사항 제목이 `[IDENTIFIER](@)` 또는 `[IDENTIFIER](path/@)`로 시작하는지 확인합니다.
...
- 본문에 등장한 식별자와 도움자가 색인에 반영되어 있는지 확인합니다.
- 색인의 제목 링크가 실제 Markdown 섹션과 VS Code 호환 anchor를 가리키는지 확인합니다.
...
- 요구사항 본문 수정 후 `@.md`, `=.md`가 함께 바뀌는 예시
