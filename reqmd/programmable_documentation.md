...
ProDoc 문서가 준수해야 할 문서 요구사항 명세와 문서 간 추적성은 ReqMd 요구사항 및 ReqMd 식별자 색인을 기준으로 관리합니다.
...
그 아래에는 문서 작성과 검증에 사용할 문서 요구사항 명세, 참고할 지식 파일, 변경 전파 대상을 기술합니다.
...
`propagation_docs`는 현재 ProDoc 문서의 변경이 영향을 줄 수 있는 다른 ProDoc 문서 목록입니다.
...
source 문서에서 발생한 변경은 target 문서의 추상화 수준, 책임 범위, `requirement_specs`가 요구하는 문서 기준에 맞게 의미를 변환한 뒤 반영해야 합니다.
...
`incoming_change`와 `changed_content`는 서로 다른 단계의 값입니다.
`incoming_change`는 현재 `document`를 수정하기 전에 주어지는 입력입니다.
최초 문서를 처리할 때는 사용자의 요청이나 작업 지시가 `incoming_change`가 되고, 전파 문서를 처리할 때는 Queue item에 담긴 이전 문서의 `changed_content`가 target 문서의 `incoming_change`가 됩니다.

`changed_content`는 UpdateContent가 끝난 뒤 현재 `document`에 실제로 반영된 변경 결과입니다.
사용자가 요청했지만 반영되지 않은 내용, 검토 필요로 남긴 내용, 영향 없음으로 판정한 내용은 `changed_content`에 포함하지 않습니다.
다음 문서로 전파할 때는 이 `changed_content`를 target 문서의 `incoming_change`로 넣고, `document`, `source_doc`, `direction`을 붙여 Queue item을 만듭니다.

따라서 값의 흐름은 다음과 같습니다.

```text
incoming_change -> UpdateContent(document, incoming_change, direction) -> changed_content
source changed_content + target document + direction + source_doc -> Queue item
Queue item.incoming_change = source changed_content
```
...
### 3. changed_content 산출

UpdateContent가 끝나면 에이전트는 문서에 실제로 반영된 변경만 `changed_content`로 산출합니다.
`changed_content`는 다음 전파 단계에서 target 문서의 `incoming_change`로 사용될 수 있어야 하므로, target 문서가 해석할 수 있는 변경 결과만 담습니다.

해당 문서에 실제 변경이 없으면 그 문서를 source로 하는 새 Queue item을 만들지 않습니다.
최초 문서에서 변경이 없으면 에이전트는 변경 없음과 검토 결과를 보고하고 작업을 종료합니다.
Queue item 처리 중 target 문서에 변경이 없으면 해당 item만 종료하고 다음 item을 처리합니다.

### 4. 전파 Queue 구성

현재 문서에 실제 변경이 있고 `propagation_docs`가 있으면, 에이전트는 전파 Queue를 구성합니다.
Queue item은 처리할 문서, 그 문서에 전달할 `incoming_change`, 변경 방향, source 문서 정보를 담는 작업 단위입니다.
...
각 Queue item은 최소한 다음 정보를 포함합니다.

- `document`: Queue item으로 처리할 문서입니다.
- `incoming_change`: 이 문서에 반영하거나 검토해야 할 변경 내용입니다.
- `direction`: `self`, `lateral`, `upstream`, `downstream` 중 하나입니다.
- `source_doc`: 전파 item인 경우 변경이 발생한 source 문서입니다.

### 5. Queue item 처리

에이전트는 Queue에서 item을 하나씩 꺼내 `document`를 처리합니다.
`document`가 ProDoc이면 해당 문서의 frontmatter를 파싱하고 `UpdateContent(document, incoming_change, direction)`을 실행합니다.
`document`가 ProDoc이 아니면 ProDoc workflow를 적용하지 않고 다음 Queue item으로 넘어갑니다.

Queue item 처리 결과는 다음 중 하나입니다.

- `document`에 변경을 반영합니다.
- 자동 변경이 위험하거나 근거가 부족하여 검토 필요 항목으로 남깁니다.
- `document`에 영향이 없다고 판정하고 그 이유를 기록합니다.

### 6. 식별자 색인 갱신

<<reqmd 는 doc 내에 여러개의 req 들이 있는 거야. prodoc 에서 incoming change는 변경이 있는 req 들이 식별이 되어야하고
  changed content 도 그것에 포함된 여러개의 req가 식별이 되어야해 그리고 incomming-changed 간에 req가 trace 되는 정보도
  식별이 되어야해. 이와 같은 관점에서 prodoc 이 부족한 점이 있는지 검토하여 업데이트해>>
...
target 문서가 ProDoc이고 실제 변경이 발생했으며 자체 `propagation_docs`를 가지고 있으면, target 문서는 다음 current ProDoc 문서가 됩니다.
에이전트는 target 문서에서 발생한 실제 변경을 새로운 `changed_content`로 산출하고, target 문서의 `propagation_docs`를 기준으로 새 Queue item을 추가합니다.

전파는 Queue가 빌 때까지 반복합니다.
다만 다음 경우에는 반복을 중단하고 사유를 보고합니다.

- 전파 대상 파일, ReqMd 식별자 색인, 문서 요구사항 ID가 존재하지 않는 경우
...
- `requirement_specs`에 선언된 ReqMd 식별자 색인과 문서 요구사항 ID가 존재한다.
