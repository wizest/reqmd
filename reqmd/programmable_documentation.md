...
- `incoming_change`: 현재 문서에 들어온 입력 변경입니다. 사용자의 요청 또는 source 문서의 변경 내용을 포함하며, 관련 RequirementSection이 식별된 경우 그 ID와 변경 내용을 함께 포함합니다.
- `changed_content`: 현재 문서 본문에 실제로 반영된 출력 변경입니다. 다음 문서로 전파할 수 있도록 변경된 RequirementSection ID와 변경 요약을 포함합니다.
- 추적 관계: 입력 RequirementSection과 출력 RequirementSection의 대응 관계입니다. upstream에서는 여러 입력이 하나의 출력으로 모일 수 있고, downstream에서는 하나의 입력이 여러 출력으로 나뉠 수 있습니다.
- UpdateContent 결과: 본문 변경 결과, 추적 관계 변경, 검토 필요 항목을 포함하는 처리 결과입니다.
- Queue item: 처리할 문서, 입력 변경, 전파 방향, source 문서를 담은 전파 작업 단위입니다. 입력 변경은 source 문서, 입력 RequirementSection 목록, RequirementSection별 변경 요약을 포함합니다.
...
4. 입력 변경에 포함된 RequirementSection ID를 기준으로 대상 RequirementSection을 식별하고, 필요한 경우 ReqMd `@.md` 색인과 원문 RequirementSection을 확인합니다.
...
7. 입력 RequirementSection과 출력 RequirementSection 사이의 추적 관계를 기록합니다.
8. 본문을 수정한 뒤, 결과가 `requirement_specs`의 기준을 만족하는지 검증합니다.
9. 기준을 만족하지 못하거나 판단이 어려운 내용은 자동으로 확정하지 않고 검토 필요 항목으로 남깁니다.

UpdateContent 결과는 본문 변경 결과, 추적 관계 변경, 검토 필요 항목으로 구분합니다.
본문이 바뀌었더라도 변경된 RequirementSection을 식별할 수 없으면 전파하지 않고 검토 필요 항목으로 남깁니다.
추적 관계 변경이 있으면 필요한 경우 ReqMd 식별자 색인을 갱신합니다.
본문 변경은 없지만 추적 관계 갱신이 필요한 경우에는 후속 전파 없이 색인 갱신만 수행할 수 있습니다.
검토 필요 항목이 발생하면 해당 item의 자동 전파는 중단하고 사유를 기록합니다.

### 3. Queue 전파

Workflow는 Queue가 빌 때까지 반복합니다.
최초 Queue item은 사용자의 요청을 `incoming_change`로 담고 `direction`을 `self`로 둡니다. 사용자의 요청이 특정 RequirementSection을 가리키지 않으면 UpdateContent 단계에서 관련 RequirementSection을 식별합니다.
본문 변경 결과는 `changed_content`로 기록하고, 전파 Queue item은 source 문서의 `changed_content`를 target 문서의 `incoming_change`로 담습니다.
전파 Queue item의 `direction`은 `lateral`, `upstream`, `downstream` 중 하나입니다.
...
- 검토 필요 항목이 발생하면 새 Queue item을 만들지 않습니다.
- 본문 변경이 없으면 새 Queue item을 만들지 않습니다.
- 본문 변경이 있고 `propagation_docs`가 있으면 target 문서별 Queue item을 추가합니다.
- 추적 관계나 ReqMd 식별자 색인만 바뀐 경우에는 Queue item을 추가하지 않고 색인 갱신 결과만 보고합니다.
...
`incoming_change`는 현재 문서에 들어온 변경의 source RequirementSection 목록을 포함합니다.
`changed_content`는 현재 문서 본문에서 실제로 변경된 output RequirementSection 목록과 변경 요약을 포함합니다.

추적 관계는 다음 사항을 표현해야 합니다.
...
다음 경우에는 해당 item의 자동 처리를 중단하고 사유를 보고합니다.

- 전파 대상 파일, ReqMd 식별자 색인, `requirement_specs`에 선언된 ID가 존재하지 않는 경우
- 자동 변경이 문서의 의도, 정책, 아키텍처 결정을 바꿀 수 있어 사용자 판단이 필요한 경우
- 동일한 Queue item이 이미 처리 이력에 기록되어 반복 처리가 되는 경우

현재 전파 경로에서 이미 처리 중인 ProDoc 문서를 다시 만나 순환 경로가 되면 전체 전파를 중단합니다.
순환 경로는 현재 전파 경로의 문서 stack으로 판단합니다.
처리 이력은 동일한 Queue item이 반복 처리되는 것을 막기 위해 사용합니다.
동일 item 여부는 최소한 처리 문서, source 문서, 전파 방향, 입력 RequirementSection 목록, 입력 변경 내용을 기준으로 판단합니다.
...
- 입력 RequirementSection, 출력 RequirementSection, 추적 관계 요약
- 갱신한 ReqMd 식별자 색인과 해당 갱신이 발생한 문서 또는 Queue item
- 전파 중단 사유
...
flowchart TD
    ...
    D --> E{"Processed item or cycle path?"}
    E -->|Yes| C
    E -->|No| F{"ProDoc document?"}
    F -->|No| C
    F -->|Yes| G["Parse frontmatter"]
    G --> H["Run UpdateContent"]
    H --> J{"Trace relation change?"}
    J -->|Yes| J2["Update ReqMd index if needed"]
    J -->|No| H2{"Content change?"}
    J2 --> H2
    H2 -->|No| C
    H2 -->|Yes| I["Prepare content change result"]
    I --> K{"Has propagation targets?"}
    K -->|No| C
...
- 입력 RequirementSection, 출력 RequirementSection, 추적 관계가 RequirementSection 단위로 식별되어 있다.
...
- 다단계 propagation에서 현재 전파 경로와 Queue item 처리 이력이 관리되어 순환 또는 반복 처리가 발생하지 않는다.
