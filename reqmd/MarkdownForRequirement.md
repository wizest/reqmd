...
- `@.md`, `=.md` 색인 파일 안의 식별자/도움자 링크는 반드시 `@#fragment`, `=#fragment`, `path/@#fragment`, `path/=#fragment`처럼 섹션 fragment를 포함합니다.
...
- `#fragment`: 색인 파일 안의 특정 섹션 위치를 지정합니다. 요구사항 본문에서는 생략할 수 있으나, `@.md` 색인 파일 안에서는 반드시 작성합니다. 일반적으로 식별자 문자열을 그대로 사용합니다.
...
색인 파일 안에서는 `[identifier](@)` 또는 `[identifier](pathto/@)`처럼 파일만 가리키는 링크를 사용하지 않습니다. 같은 색인 파일의 다른 섹션은 `[identifier](@#identifier)`로, 다른 경로의 색인 섹션은 `[identifier](pathto/@#identifier)`로 작성합니다. 이는 VS Code 같은 Markdown 도구에서 링크를 따라 정의 섹션으로 바로 이동할 수 있게 하기 위한 규칙입니다.
...
- `#fragment`: 색인 파일 안의 특정 섹션 위치를 지정합니다. 요구사항 본문에서는 생략할 수 있으나, `=.md` 색인 파일 안에서는 반드시 작성합니다. 일반적으로 도움자 문자열을 그대로 사용합니다.
...
색인 파일 안에서는 `[helper](=)` 또는 `[helper](path/=)`처럼 파일만 가리키는 링크를 사용하지 않습니다. 같은 색인 파일의 다른 섹션은 `[helper](=#helper)`로, 다른 경로의 색인 섹션은 `[helper](path/=#helper)`로 작성합니다. 구현 도움자도 같은 규칙을 따르며, 예를 들어 `[=BrakeLampReq](=#=BrakeLampReq)`처럼 실제 구현 이름을 fragment에 보존합니다.
...
식별자 색인 섹션에 포함된 링크는 현재 섹션의 식별자와 관계가 있는 항목입니다. 원문 문서를 가리키는 링크도 고정 라벨 대신 현재 식별자 문자열을 그대로 표시합니다. 색인 파일 안에서 다른 색인 엔트리를 가리키는 링크는 반드시 fragment를 포함합니다. 관계의 종류는 링크 대상의 경로, 문서 위치, 주변 설명, 프로젝트 맥락으로 해석합니다. 색인 파일은 기본적으로 관계 타입을 강제하지 않습니다.
...
도움자 색인 섹션에 포함된 링크는 현재 섹션의 도움자와 연결된 항목입니다. 원문 문서를 가리키는 링크도 고정 라벨 대신 현재 도움자 문자열을 그대로 표시합니다. 색인 파일 안에서 다른 색인 엔트리를 가리키는 링크는 반드시 fragment를 포함합니다. 연결의 종류는 링크 대상의 경로, 문서 위치, 주변 설명, 프로젝트 맥락으로 해석합니다. 색인 파일은 기본적으로 연결 타입을 강제하지 않습니다.
...
- `@.md` 또는 `=.md`의 원문 문서 링크가 실제 문서 위치를 가리키는지 확인합니다.
...
6. `@.md`에 새 식별자 섹션을 만들고 원문 문서 링크를 추가합니다.
...
3. 원문 문서 링크는 `[원문]`이 아니라 실제 식별자 또는 도움자 문자열로 작성합니다.
...
- 색인의 원문 문서 링크가 실제 Markdown 섹션을 가리키는지 확인합니다.
