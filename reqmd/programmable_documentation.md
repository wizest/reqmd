...
문서에 포함된 메타데이터는 에이전트가 어떤 요구사항을 만족해야 하는지, 어떤 지식을 참고해야 하는지, 변경을 어느 문서로 전파해야 하는지를 결정합니다.
...
이 문서에서 `requirement_refs`가 가리키는 요구사항은 ProDoc 문서가 산출물로서 준수해야 하는 문서 요구사항입니다.
...
참조 파일이나 요구사항 ID가 존재하지 않으면 에이전트는 임의의 대체 대상을 만들지 않고 검토 필요 항목으로 보고합니다.
빈 목록은 "선언되었지만 대상 없음"으로 처리하고, 선택 항목이 누락되면 해당 단계가 없는 것으로 처리합니다.

### requirement_refs

`requirement_refs`는 ProDoc 문서가 산출물로서 준수해야 하는 ReqMd 요구사항 식별자 목록입니다.
이 요구사항은 문서가 설명하는 제품이나 시스템 자체의 요구사항과 엄격하게 구분됩니다.
예를 들어 ProDoc 문서가 SW 상세 설계를 설명한다면, `requirement_refs`는 소프트웨어 기능 요구사항 목록이 아니라 SW 상세 설계서가 갖추어야 할 구조, 필수 내용, 품질, 추적성 기준을 가리킵니다.

각 항목은 ReqMd identifier index 경로를 키로 사용하고, 그 아래에 참조할 요구사항 ID를 나열합니다.
ProDoc 문서 작성 및 검증에서 `requirement_refs`는 필수 항목입니다.
...
`requirement_refs`는 문서가 반드시 만족해야 하는 기준이고, `knowledge_files`는 그 기준을 만족하는 내용을 작성하기 위한 근거와 문맥입니다.
...
실제 수정 여부는 Workflow에서 `changed_content`, target 문서의 `requirement_refs`, target 문서의 `knowledge_files`를 함께 검토하여 결정합니다.
...
- target 문서의 `requirement_refs`가 요구하는 구조, 품질, 추적성 조건을 만족하도록 작성합니다.
...
해당 키가 있으면 ProDoc workflow를 적용하고 `requirement_refs`, `knowledge_files`, `propagation_docs`를 파싱합니다.

이 파싱은 최초 문서에서만 수행하지 않습니다.
전파 Queue에서 target 문서를 꺼내 처리할 때도 target 문서의 frontmatter를 다시 파싱합니다.
각 문서는 자신이 선언한 `requirement_refs`와 `knowledge_files`를 기준으로 업데이트되어야 하기 때문입니다.
...
1. `document`의 `requirement_refs`를 읽고, ReqMd 식별자 색인과 요구사항 원문을 조회합니다.
...
5. 본문을 수정한 뒤, 결과가 `requirement_refs`의 기준을 만족하는지 검증합니다.
