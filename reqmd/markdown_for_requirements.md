...
| 요구사항 본문(RequirementBody) | EARS 규칙을 적용하여 요구사항의 의도, 조건, 기대 결과를 작성하는 Markdown 본문 |
...
`식별자 색인` 은 Markdown 파일로 작성되며, 각 식별자의 관계(relationship)와 그 요구사항에서 사용한 도움자를 보여줍니다. 이 색인을 보면 요구사항이 어떤 다른 요구사항과 연결되는지, 어떤 도움자를 사용하는지 알 수 있습니다. 속성의 원천 정보는 요구사항 속성(RequirementAttributes)에만 두고, 식별자 색인에는 YAML 속성자를 작성하지 않습니다.
...
- [used_helper_in_other_path](pathto/=#used_helper_in_other_path)
````

식별자 색인 섹션 제목은 원래 요구사항 위치를 가리키는 링크로 작성합니다. 제목 링크는 **원문 요구사항 섹션(RequirementSection)**으로만 이동해야 합니다.

섹션 본문 목록에는 원문 링크를 반복하지 않습니다. 대신 현재 식별자와 관련된 다른 식별자, 그리고 현재 요구사항에서 사용한 도움자를 적습니다. 각 항목은 해당 식별자 또는 도움자의 **색인 섹션**을 가리켜야 합니다. 예를 들어 대상 식별자 색인 섹션 제목이 `## [SYS_BLC_CONTROL](SysReq.md#sys_blc_control-브레이크-램프-제어-시스템)`이면 목록 항목은 `[SYS_BLC_CONTROL](../sys/@#sys_blc_control)`처럼 작성합니다. 대상 도움자 색인 섹션 제목이 `## brake_lamp_request`이면 목록 항목은 `[brake_lamp_request](=#brake_lamp_request)`처럼 작성합니다.

즉, 식별자 색인에서 `## [SW_BLC_CONTROL](SwReq.md#...)` 같은 **제목 링크**는 원문 요구사항으로 이동합니다. 목록의 `- [SYS_BLC_CONTROL](../sys/@#sys_blc_control)` 같은 **관계 링크**는 다른 식별자의 색인 섹션으로 이동하고, `- [brake_lamp_request](=#brake_lamp_request)` 같은 **도움자 링크**는 사용한 도움자의 색인 섹션으로 이동합니다.
...
어떤 도움자를 사용했는지는 요구사항 본문(RequirementBody)에서 확인합니다. 요구사항 섹션에서 도움자가 사용되면, 그 식별자 색인 섹션에 도움자 색인 링크를 함께 넣습니다. 일반 섹션(GeneralSection)에만 등장한 도움자는 식별자 색인에 포함하지 않습니다.
...
`도움자 색인` 은 Markdown 파일로 작성되며, 각 도움자의 연결(mapping)과 그 도움자를 사용한 식별자를 보여줍니다. 이 색인을 보면 도움자가 어떤 다른 도움자와 연결되는지, 어떤 요구사항에서 사용되는지 알 수 있습니다. 도움자에 대한 속성이나 정의가 필요하면 원문 요구사항 섹션(RequirementSection), 설계 문서, 테스트 문서 같은 본문에 작성하고, 도움자 색인에는 YAML 속성자를 작성하지 않습니다.
...
- [USING_IDENTIFIER](@#using_identifier)
- [USING_IDENTIFIER_IN_OTHER_PATH](pathto/@#using_identifier_in_other_path)
````

도움자 색인 섹션 제목은 링크 없이 도움자 이름만 작성합니다. 도움자는 여러 요구사항이나 여러 원문 위치에서 사용될 수 있으므로, 섹션 제목에서 하나의 원문 위치를 고르면 안 됩니다.

섹션 본문 목록에는 원문 링크를 반복하지 않습니다. 대신 현재 도움자와 연결되는 다른 도움자, 그리고 현재 도움자를 사용한 식별자를 적습니다. 각 항목은 해당 도움자 또는 식별자의 **색인 섹션**을 가리켜야 합니다. 예를 들어 대상 도움자 색인 섹션 제목이 `## brake_lamp_request_signal`이면 목록 항목은 `[brake_lamp_request_signal](../swdd/=#brake_lamp_request_signal)`처럼 작성합니다. 현재 도움자가 `SW_BLC_CONTROL` 요구사항에서 사용되면 목록 항목은 `[SW_BLC_CONTROL](../sw/@#sw_blc_control)`처럼 작성합니다.

즉, 도움자 색인에서 `## brake_lamp_request` 같은 **섹션 제목**은 링크가 아닙니다. 목록의 `- [brake_lamp_request_signal](../swdd/=#...)` 같은 **연결 링크**는 다른 도움자의 색인 섹션으로 이동하고, `- [SW_BLC_CONTROL](../sw/@#sw_blc_control)` 같은 **식별자 링크**는 이 도움자를 사용한 식별자의 색인 섹션으로 이동합니다.
...
어떤 식별자가 도움자를 사용했는지는 요구사항 본문(RequirementBody)에서 확인합니다. 요구사항 섹션에서 도움자가 사용되면, 그 도움자 색인 섹션에 요구사항 식별자 링크를 함께 넣습니다. 일반 섹션(GeneralSection)에만 등장한 도움자는 도움자 색인의 식별자 링크를 만들지 않습니다.
...
RequirementBody
  EARSRequirement
  EARSRequirement newline RequirementBody

EARSRequirement
  UbiquitousRequirement
  EventDrivenRequirement
  StateDrivenRequirement
  OptionalFeatureRequirement
  UnwantedBehaviorRequirement

UbiquitousRequirement
  "<Subject> shall <Response>"

EventDrivenRequirement
  "When <Trigger>, <Subject> shall <Response>"

StateDrivenRequirement
  "While <State>, <Subject> shall <Response>"

OptionalFeatureRequirement
  "Where <Feature>, <Subject> shall <Response>"

UnwantedBehaviorRequirement
  "If <Condition>, then <Subject> shall <Response>"
...
- 상태 기반 요구사항: 특정 상태가 유지되는 동안의 동작을 작성합니다.
- 선택 기능 요구사항: 특정 기능이나 옵션이 존재하는 경우의 동작을 작성합니다.
- 예외 또는 원하지 않는 동작 요구사항: 오류, 예외, 비정상 조건에서의 응답을 작성합니다.
...
- `@.md`: 식별자 색인 파일입니다. 요구사항 식별자의 관계 링크와 그 식별자가 사용한 도움자 링크를 포함합니다.
- `=.md`: 도움자 색인 파일입니다. 도움자의 연결 링크와 그 도움자를 사용한 식별자 링크를 포함합니다.
...
- 요구사항 본문(RequirementBody)은 EARS 규칙을 적용하여 요구사항의 의도, 조건, 기대 결과를 명확히 적습니다.
- 요구사항 본문(RequirementBody)은 가능한 한 `항상`, `...할 때`, `...인 경우`, `...동안`, `...가 제공되는 경우`처럼 적용 조건과 기대 응답이 드러나는 문장으로 작성합니다.
...
`@.md`의 각 섹션은 하나의 요구사항 섹션(RequirementSection) 식별자를 기준으로 작성합니다. 일반 섹션(GeneralSection)은 포함하지 않습니다. 섹션 본문 목록에는 현재 식별자와 관련된 다른 식별자와 현재 요구사항에서 사용한 도움자를 함께 작성합니다.
...
`=.md`의 각 섹션은 하나의 도움자 이름을 기준으로 작성합니다. 도움자는 여러 요구사항에서 사용될 수 있으므로 섹션 제목에는 링크를 걸지 않습니다. 일반 섹션(GeneralSection)에만 등장하는 도움자는 포함하지 않습니다. 섹션 본문 목록에는 현재 도움자와 연결되는 다른 도움자와 현재 도움자를 사용한 식별자를 함께 작성합니다.
...
`@.md` 색인 섹션의 제목 링크는 원문 요구사항 섹션을 가리킵니다. `=.md` 색인 섹션의 제목은 링크 없이 도움자 이름만 적습니다. 색인 섹션 본문의 목록 링크는 관계 항목, 연결 항목, 사용 항목의 색인 섹션을 가리킵니다. 본문 목록 링크는 fragment를 포함해야 합니다. 관계의 종류는 링크 대상(`@` 또는 `=`), 경로, 주변 맥락으로 해석합니다.
...
- `@.md`, `=.md` 색인 파일 안의 식별자/도움자 링크가 단순 `@`, `=`, `path/@`, `path/=`가 아니라 fragment를 포함한 `@#...`, `=#...`, `path/@#...`, `path/=#...` 형식인지 확인합니다. `@.md`의 도움자 링크와 `=.md`의 식별자 링크에도 같은 규칙을 적용합니다.

### 추적성 검증

- 요구사항 본문(RequirementBody)에 등장한 식별자가 `@.md`에 필요한 관계로 반영되었는지 확인합니다.
- 요구사항 본문(RequirementBody)에 등장한 도움자가 `=.md`에 필요한 연결로 반영되었는지 확인합니다.
- 요구사항 본문(RequirementBody)에 등장한 도움자가 해당 요구사항 식별자의 `@.md` 색인 섹션에 반영되었는지 확인합니다.
- 요구사항 본문(RequirementBody)에 등장한 도움자의 `=.md` 색인 섹션에 해당 요구사항 식별자가 반영되었는지 확인합니다.
- `@.md`의 제목 링크가 실제 원문 요구사항 위치와 원문 heading anchor를 정확히 가리키는지 확인합니다.
- `=.md`의 도움자 색인 섹션 제목에 링크가 없는지 확인합니다.
...
- 요구사항 본문(RequirementBody)이 EARS 규칙에 따라 적용 조건과 기대 응답을 구분해 표현하는지 확인합니다.
...
- `@.md`의 제목 링크가 실제 원문 요구사항 섹션과 VS Code 호환 원문 heading anchor를 가리키는지 확인합니다.
- `=.md`의 도움자 색인 섹션 제목에 링크가 없는지 확인합니다.
