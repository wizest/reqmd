...
식별자 색인 섹션에 포함된 링크는 현재 섹션의 식별자와 관계가 있는 항목입니다. 단순 목록으로 작성하면 관계 종류를 구분하지 않은 일반 관계로 해석합니다. 관계 종류를 구분해야 하면 `Parent`, `Child`, `DerivedFrom`, `VerifiedBy`, `Related` 같은 그룹 제목을 사용합니다.
...
식별자 간의 구체적 관계는 경로와 그룹 제목을 함께 활용하여 특정합니다. 예를 들어 `sysrq`, `swrq`, `swit`, `swqt`, `sysqt` 같은 요구사항 경로를 사용하면 현재 식별자의 상위 요구사항, 하위 요구사항, 통합 테스트, 소프트웨어 테스트와의 관계를 명시적으로 구분할 수 있습니다.
...
도움자 색인 섹션에 포함된 링크는 현재 섹션의 도움자와 연결된 항목입니다. 단순 목록으로 작성하면 연결 종류를 구분하지 않은 일반 연결로 해석합니다. 연결 종류를 구분해야 하면 `Abstract`, `Concrete`, `ImplementedBy`, `ObservedBy`, `Alias` 같은 그룹 제목을 사용합니다.
...
도움자 간의 구체적 연결은 경로와 그룹 제목을 함께 활용하여 특정합니다. 예를 들어 `sysrq`, `swrq`, `swit`, `swqt`, `sysqt` 같은 요구사항 경로를 사용하면 현재 도움자의 상위 개념, 하위 구현 대상, 테스트 관찰값과의 연결을 명시적으로 구분할 수 있습니다.
...
색인 섹션에 포함된 링크는 모두 현재 섹션의 식별자 또는 도움자와 관계가 있는 항목으로 해석합니다. 관계의 종류가 필요한 프로젝트에서는 링크를 그룹 제목으로 구분합니다.
...
ReqMd 문서를 수정한 뒤에는 다음 항목을 확인합니다. 이 목록은 나중에 skill의 검증 절차 또는 스크립트 입력으로 사용할 수 있습니다.

### 문법 검증

- 모든 요구사항 섹션 제목이 `[identifier](@)`로 시작하는지 확인합니다.
...
- 상위 요구사항과 하위 요구사항의 관계가 양방향으로 추적 가능한지 확인합니다.
