...
4. Run `python claude\prodoc\scripts\validate_prodoc.py <file-or-root>` before finishing when files are available.
5. Run `python claude\prodoc\scripts\extract_sections.py <prodoc.md>` before editing body sections.
6. Run `python claude\prodoc\scripts\resolve_specs.py <prodoc.md>` before reading requirement specs.
7. Use `python claude\prodoc\scripts\plan_propagation.py <root> <start-doc> --input-section <REQ_ID> --summary "<change>"` before propagation.
8. Use the ReqMd skill and `python claude\reqmd\scripts\reqmd_fix.py <requirement-root>` for any ReqMd RequirementSection or `@.md` index edit.
...
Validate saved result reports with `python claude\prodoc\scripts\validate_update_result.py <result.md> --source <source.md> --target <target.md>`.
...
1. Run `python claude\prodoc\scripts\plan_propagation.py <root> <start-doc> --input-section <REQ_ID> --summary "<change>"`.
...
- run `python claude\reqmd\scripts\reqmd_fix.py <requirement-root>` after ReqMd edits;
...
- Run `python claude\prodoc\scripts\extract_sections.py <prodoc.md>` when body RequirementSections changed.
- Run `python claude\prodoc\scripts\resolve_specs.py <prodoc.md>` when requirement specs guide the edit.
- Run `python claude\prodoc\scripts\validate_update_result.py <result.md> --source <source.md> --target <target.md>` when a result report is saved.
